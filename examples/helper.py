...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Run demo of binary classification")

    parser.add_argument("--http_proxy", action="store", required=False, help="HTTP Proxy", default=None)

    parser.add_argument("--https_proxy", action="store", required=False, help="HTTPS Proxy", default=None)
...
def one_hot_encode(data, columns):
    ...
    for c in columns:
        ...
        data = np.hstack((ensure_matrix(data[:, :c]), ensure_matrix(one_hot), ensure_matrix(data[:, c + 1 :])))
...
def load_dermatology(proxies):
    # Dermatology
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/dermatology/dermatology.data", proxies=proxies
    ).text
    lines = text.split("\n")
...
def load_diabetic(proxies):
    ...
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/00329/messidor_features.arff", proxies=proxies
    ).text
...
def load_eeg(proxies):
    # load EEG eye data
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/00264/EEG%20Eye%20State.arff", proxies=proxies
    ).text
    text = text[text.index("@DATA") :]
    lines = text.split("\n")[1:]
    train = np.vstack([np.fromstring(lines[i], sep=",") for i in range(len(lines) - 1)])
...
def load_gamma(proxies):
    ...
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/magic/magic04.data", proxies=proxies
    ).text
...
def load_glass(proxies):
    ...
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/glass/glass.data", proxies=proxies
    ).text
    lines = text.split("\n")
    train = np.vstack([np.fromstring(lines[i], sep=",") for i in range(len(lines) - 1)])
...
def load_haberman(proxies):
    ...
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/haberman/haberman.data", proxies=proxies
    ).text
    lines = text.split("\n")
    train = np.vstack([np.fromstring(lines[i], sep=",") for i in range(len(lines) - 1)])
    train[:, -1] -= 1
...
def load_heart(proxies):
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/statlog/heart/heart.dat", proxies=proxies
    ).text
    lines = text.split("\n")
    train = np.vstack([np.fromstring(lines[i], sep=" ") for i in range(len(lines) - 1)])
...
def load_seeds(proxies):
    ...
    train = parse_ripley(
        requests.get(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/00236/seeds_dataset.txt", proxies=proxies
        ).text
    )
    train[:, -1] -= 1
    test = train
...
def load_seismic(proxies):
    ...
    text = requests.get(
        "https://archive.ics.uci.edu/ml/machine-learning-databases/00266/seismic-bumps.arff", proxies=proxies
    ).text
    text = text[text.index("@data") :]
...
def draw_node_2d_perpendicular(node, bounds, colormap, n_classes):
    if node.is_leaf():
        ...
        draw_node_2d_perpendicular(
            node.child1_, compute_child_bounds_2d_perpendicular(bounds, node, True), colormap, n_classes
        )
        draw_node_2d_perpendicular(
            node.child2_, compute_child_bounds_2d_perpendicular(bounds, node, False), colormap, n_classes
        )
...
def plot_root(root, X, y, title, cmap):
    ...
    def plot_node(node, node_vs_color={}, level=0, parents=[], side=None):
        ...
        if node.child1_:
            plot_node(
                node.child1_, node_vs_color, level + 1, parents=parents + [Parent(line, origin, normal, "L")], side="L"
            )

        if node.child2_:
            plot_node(
                node.child2_, node_vs_color, level + 1, parents=parents + [Parent(line, origin, normal, "R")], side="R"
            )
