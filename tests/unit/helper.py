...
from bayesian_decision_tree.classification import HyperplaneClassificationTree, PerpendicularClassificationTree
...
from bayesian_decision_tree.regression import HyperplaneRegressionTree, PerpendicularRegressionTree
...
def create_classification_trees(prior, partition_prior, prune=False):
    return [
        ...
        HyperplaneClassificationTree(
            partition_prior, prior, delta=0, prune=prune, optimizer=ScipyOptimizer(DifferentialEvolutionSolver, 666)
        ),
        HyperplaneClassificationTree(
            partition_prior, prior, delta=0, prune=prune, optimizer=RandomTwoPointOptimizer(100, 666)
        ),
        HyperplaneClassificationTree(
            partition_prior, prior, delta=0, prune=prune, optimizer=RandomHyperplaneOptimizer(100, 666)
        ),
        HyperplaneClassificationTree(
            partition_prior, prior, delta=0, prune=prune, optimizer=SimulatedAnnealingOptimizer(10, 10, 0.9, 666)
        ),
    ]
